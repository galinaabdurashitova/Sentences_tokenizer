...
def tokenizer(text):
    ...
    for i in range(len(lines) - 1):  # Looking at the pair of elements
        if lines[i + 1][0] not in capitals:  # If the next element doesn't look like a beginning of a sentence
            line = line + ' ' + lines[i + 1]  # It should be joined to the previous part
        else:
            sentences.append(line)
            line = lines[i + 1]  # Otherwise remember the previous sentence and start the new one
    return sentences
